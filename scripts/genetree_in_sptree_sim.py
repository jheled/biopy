...
parser = optparse.OptionParser("""%prog [OPTIONS] tree-or-trees-file

  Generate gene trees compatible with species tree. The species trees in the
  NEXUS input file should contain population size information in the same format
  generated by *BEAST. Tips of invididuals belonging to 'species-name' are
  labeled 'species-name_tip0', 'species-name_tip1', etc. in the gene tree.""")

parser.add_option("-n", "--ntrees", dest="ngenetrees", metavar='N',
                  help="""Generate N gene trees per species tree """
                  + """(default %default).""", default = "1") 

parser.add_option("-t", "--per-species", dest="ntips", metavar='N',
                  help="""N individuals (lineages) per species """
                  + """(default %default).""", default = "2") 

parser.add_option("-o", "--nexus", dest="nexfile", metavar="FILE",
                  help="Output trees in nexus format to FILE.", default = None)
...
if os.path.isfile(nexusTreesFileName) :
  trees = INexus.INexus().read(fileFromName(nexusTreesFileName))
...
for tree in trees:
  ...
  for k in range(nGeneTrees) :
    ...
    tlog.outTree(toNewick(g),
                 name = (tree.name + ('_%d' % k)) if tree.name else None)
