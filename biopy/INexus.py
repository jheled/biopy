...
class INexus(object):
    ...
    def read(self,input, sample = None):
        ...
        while 1:
            ...
            if title in KNOWN_NEXUS_BLOCKS:
              ii = self._parse_nexus_block(title, contents)
              if ii :
                for i in ii:
                  yield i
            else:
    ...
    def _tree(self,options):
        slc = self.sample[1]
        if slc is not None :
          count = self.sample[0] + 1
          self.sample[0] = count
          if not ((slc.start <= count) and (slc.stop == -1 or count < slc.stop)
                  and ((count - slc.start) % slc.step) == 0) :
            #opts.rest()
            return

        opts=CharBuffer(options)
        name=opts.next_word()
        # Ignore tree comment, if any 
        if opts.peek_nonwhitespace() == '[':
            if not opts.next_until(']'):
                raise NexusError,'Syntax error in tree description: unmached \[ %s' % options[:50]
            cc = opts.next_nonwhitespace();
        ...
        if self.translate:
            for n in tree.get_terminals():
                ...
                try:
                    data.taxon = safename(self.translate[int(data.taxon)])
                except (ValueError,KeyError):
                    raise NexusError,'Unable to substitue %s using \'translate\'data.' % data.taxon
        yield tree
